...
class RunlogsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create runlog" do
    assert_difference('Runlog.count') do
      post runlogs_url, params: { runlog: { address: @runlog.address, date: @runlog.date, run: @runlog.run } }
    end
  ...
  test "should update runlog" do
    patch runlog_url(@runlog), params: { runlog: { address: @runlog.address, date: @runlog.date, run: @runlog.run } }
